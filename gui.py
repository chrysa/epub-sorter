...
class Gui(Common):
    def __init__(self, args):
        ...
        self.args = args
        self.root = tk.Tk()
        self.root.title("Epub Orderer GUI")
        ...
        self.epub_path = self.args.epub_path
        self.csv_output = self.args.output_csv
        self.current_step = tk.StringVar(value="")
        self.processed_folder = self.args.processed_folder
        self.duplicate_folder = self.args.duplicate_folder
        self.failed_folder = self.args.failed_folder
    ...
    def create_widgets(self):
        ...
        # tk.Checkbutton(self.root, text="Update Title Metadata", variable=self.update_title_var).grid(row=5, column=3, sticky="w")
        tk.Label(self.root, text="Files Processed:").grid(row=6, column=0, sticky="w")
        ...
        self.total_files_label.grid(row=7, column=0, sticky="w")
        self.current_file_index_label = tk.Label(
            self.root, text="Current File Index: 0",
        )
        self.current_file_index_label.grid(row=7, column=1, columnspan=3, sticky="w")

        self.current_step_label = tk.Label(self.root, textvariable=self.current_step)
        self.current_step_label.grid(row=8, column=0, columnspan=4, pady=5)

        self.progress_bar = ttk.Progressbar(
            self.root, orient="horizontal", length=400, mode="determinate",
        )
        self.progress_bar.grid(row=9, column=0, columnspan=4, pady=10)
    ...
    def browse_epub_path(self):
        if (folder_selected := filedialog.askdirectory()):
            self.epub_path.set(folder_selected)
    ...
    def run_processing(self):
        ...
        if self.epub_list:
            ...
            for index, epub in enumerate(self.epub_list):
                self.current_step.set(f"extract metadata {epub}")
            ...
            if self.update_var.get() or self.rename_file_var.get():
                for epub in self.processed_folder.rglob("*.epub"):
                    self.current_step.set(f"rename {epub}")
